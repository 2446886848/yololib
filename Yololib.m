...
void inject_dylib(FILE* newFile, uint32_t top, NSString *libPath) {
    fseek(newFile, top, SEEK_SET);
    struct mach_header mach;
    
    fread(&mach, sizeof(struct mach_header), 1, newFile);
    
    NSString *libName = [NSString stringWithFormat:@"@executable_path/%@", libPath.lastPathComponent];
    NSLog(@"libName  = %@", libName);
    NSData* data = [libName dataUsingEncoding:NSUTF8StringEncoding];
...
void inject_dylib_64(FILE* newFile, uint32_t top, NSString *libPath) {
    @autoreleasepool {
        fseek(newFile, top, SEEK_SET);
        ...
        unsigned long dylib_size = sizeof(struct dylib_command) + b_round(strlen([libPath UTF8String]) + 1, 8);
...
void inject_file(NSString* file, NSString* libPath)
{
    char buffer[4096], binary[4096];
    
    strlcpy(binary, [file UTF8String], sizeof(binary));
    
    NSLog(@"dylib path %@", libPath);
    ...
    switch (fh->magic) {
        ...
        case FAT_MAGIC:
        {
            ...
            for (i = 0; i < CFSwapInt32(fh->nfat_arch); i++) {
                ...
                if (CFSwapInt32(arch->cputype) == CPU_TYPE_ARM64) {
                    ...
                    inject_dylib_64(binaryFile, CFSwapInt32(arch->offset), libPath);
                }
                else {
                    inject_dylib(binaryFile, CFSwapInt32(arch->offset), libPath);
                }
                arch++;
            }
            break;
        ...
        case MH_MAGIC_64:
        {
            ...
            inject_dylib_64(binaryFile, 0, libPath);
            break;
        }
        case MH_CIGAM:
        case MH_MAGIC:
...
@implementation Yololib

+ (BOOL)injectFile:(NSString *)filePath libPath:(NSString *)libPath
{
    NSParameterAssert(filePath);
    NSParameterAssert(libPath);
    inject_file(filePath, libPath);
    return YES;
}
